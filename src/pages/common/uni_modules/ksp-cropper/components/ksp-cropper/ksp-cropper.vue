...
<view v-if="url" :mode="modeValue" :change:mode="mwx.changeMode" :rotate="rotate" :change:rotate="mwx.changeRotate">
	...
	<view class="panel">
		<view class="body" @touchstart="mwx.touchstart" @touchmove="mwx.touchmove" @touchend="mwx.touchend" @touchcancel="mwx.touchcancel">
			...
			<view class="mask" :style="{'--mask-bg-col': maskBgColor, '--mask-opacity': maskOpacity}"></view>
			<view class="frame" :class="{transit: transit}" :change:rect="mwx.changeFrame" :rect="frame" :style="{ '--btn-font-size': `${btnFontSize}rpx`, '--crop-corner-color': cropCornerColor, '--crop-corner-border': cropCornerBorder }">
				<view class="rect" :style="{'--image-border-size': imageBorderSize}">
		...
		<view v-if="showDefaultTool" class="toolbar" :style="{ '--btn-font-size': `${btnFontSize}rpx`}">
...
export default {
	props: {
		...
		/**
		 * 是否在微信中使用 OffscreenCanvas 进行绘制。
		 * 注意⚠️：由于微信对临时文件夹的限制（无法写入），这个方法取到的图片，需要马上保存到另外的地方，否则下次绘制时会被清空。
		 */
		useWxOffscreenCanvas: {
			type: Boolean,
			default: true,
		},
    btnFontSize: {
	...
	mounted() {
		...
		this.body.width = info.windowWidth;
		this.body.height = info.windowHeight;
		this.init();
	},
	methods: {
		imageLoad(event) {
		...
		init() {
			...
			var width = this.body.width * this.displayScale;
			var height = this.body.height * this.displayScale;
		...
		async cropWx() {
			...
			/**
			 * 使用微信的 OffscreenCanvas，避免有些真机在加载大图的时候，canvas.createImage() 加载图片的时候变成全白。
			 * iPhone 12 Pro Max， 宽高超过 1800 的时候，会加载成白色。限制成微信推荐的 1363 * 1363， 某些机型还是会有问题。
			 * 参考： https://developers.weixin.qq.com/community/develop/doc/0002e0a8754860acd8ce9c34651000?_at=1676846828354
			 * 尝试过
			 * 	1. 用 OffscreenCanvas 绘制，context.getImageData 导出数据后，wx.canvasPutImageData 将数据放到 canvas 组件中，wx.canvasToTempFilePath 依然拿到空白图，canvas.toDataUrl 拿到的数据是有的，但是应该是空白图的
			 */
			if (this.useWxOffscreenCanvas) {
				uni.showLoading({
					title: "处理中"
				});

				const failureHandling = (title) => {
					uni.hideLoading()
					uni.showToast({
						title: title,
						icon: 'error',
				...
				const urlToDraw = this.url;

				// 删除旧的生产的图片，由于微信不能操作临时文件夹，只能先放到用户文件夹，然后自动清理
				const fs = wx.getFileSystemManager()
				const tempDir = wx.env.USER_DATA_PATH + '/ksp-crop-temp/'
				...
				// })
				/**
				 * 部分机型 rmdir 会出现无法回调的情况 ，使用 unlinkSync。https://blog.csdn.net/nju_zjy/article/details/127125596
				 */
				await new Promise((resolve, reject) => {
					fs.readdir({
						dirPath: tempDir,
						success(list) {
							list.files.forEach((fName) => {
								const fPath = `${tempDir}/${fName}`;
								try {
									fs.unlinkSync(fPath);
								} catch (e) {
									console.error(e);
				...
				const mkdirRes = await new Promise((resolve, reject) => {
					fs.mkdir({
						dirPath: tempDir,
						recursive: true,
						success: (res) => {
						...
						fail: (e) => {
							console.error('canvas create dir failed', e)
							if (e.errMsg.includes('already exists')) {
								resolve(e)
							} else {
								resolve();
							}
				...
				// 创建离屏 2D canvas 实例
				const canvas = wx.createOffscreenCanvas({ type: '2d', width: mx.tw, height: mx.th })
				var context = canvas.getContext("2d");
				var image = canvas.createImage();
				const loadRes = await new Promise((resolve, reject) => {
					image.onload = (res) => {
						console.log('canvas create image onload', res)
						resolve(res || true)
					};
					image.onerror = (e) => {
						console.error('canvas create image onerror', e)
						resolve()
					};
					image.src = urlToDraw;
				}).catch((e) => {
					console.log('canvas createImage', e)
				});
				if (loadRes == undefined) {
					failureHandling('图片加载失败');
					return;
				}
				...
				if (imageData == undefined || imageData === 'data,') {
					failureHandling('图片生成失败');
					return;
				}

				var name = new Date().getTime();
				let toSavePath = tempDir + name + '.jpg';
				const savedPath = await new Promise((resolve, reject) => {
					fs.writeFile({
						...
						encoding: 'base64',
						success: res => {
							console.log('writeFile success', res, toSavePath)

							// 通过微信的 API 将图片放置在临时文件夹中，防止用户文件夹过大
							uni.compressImage({
								src: toSavePath,
								quality: 100,
				...
				if (savedPath == undefined || savedPath.length < 0) {
					failureHandling('保存文件失败');
					return;
				}

				// 检查文件是否存在，防止意外删除的情况发生
				await new Promise((resolve, reject) => {
					uni.getImageInfo({
						src: savedPath,
						success: (rst) => {
							console.log('getImageInfo success, file exist', rst)
						},
						fail: (e) => {
							console.error('getImageInfo failed, maybe multi thread deletion?', e)
						},
						complete: resolve,
					});
				})
...
.rect {
	position: absolute;
	left: calc(0 - var(--image-border-size));
	top: calc(0 - var(--image-border-size));
	width: 100%;
	height: 100%;
...
.frame-left-top {
	...
	left: -6px;
	top: -6px;
	border-left: var(--crop-corner-border) solid var(--crop-corner-color);
	border-top: var(--crop-corner-border) solid var(--crop-corner-color);
	box-sizing:content-box;
}
.frame-left-bottom {
	position: absolute;
	...
	bottom: -6px;
	border-left: var(--crop-corner-border) solid var(--crop-corner-color);
	border-bottom: var(--crop-corner-border) solid var(--crop-corner-color);
	box-sizing:content-box;
}
.frame-right-top {
	position: absolute;
	...
	right: -6px;
	top: -6px;
	border-right: var(--crop-corner-border) solid var(--crop-corner-color);
	border-top: var(--crop-corner-border) solid var(--crop-corner-color);
	box-sizing:content-box;
}
.frame-right-bottom {
