...
export default {
  ...
  computed: {
    ...
    tabItemStyle() {
      return (index) => {
        ...
        //   style['margin-left'] = `${this.gutter}rpx`
        // }        
        if (index != 0) {
          style['margin-left'] = `${this.gutter}rpx`
        }
  ...
  methods: {
    ...
    async init() {
      // 获取tabs组件的尺寸信息
      let tabRect = await this.getRect('#' + this.id, false)
      // tabs组件距离屏幕左边的宽度
    ...
    scrollByIndex() {
      ...
      // 计算当前活跃item到组件左边的距离
      if (this.current !== 0) {
        const diff = tabWidth - uni.upx2px(this.barWidth)
        console.log('diff~ ', diff)
        this.scrollBarLeft = left - uni.upx2px(this.barWidth) / 2 - diff
      } else {
